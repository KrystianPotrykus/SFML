        ...
        if (connectedCount > openedCount)
        {
            ...
            if (devices != NULL)
            {
                ...
                if (size > 0)
                {
                    ...
                    CFSetGetValues(devices, &array[0]);
                    ...
                    {
                        IOHIDDeviceRef d = static_cast<IOHIDDeviceRef>(const_cast<void*>(array[static_cast<size_t>(didx)]));
...
bool JoystickImpl::open(unsigned int index)
{
    ...
    CFSetGetValues(devices, &devicesArray[0]);

    // Get the desired joystick.
    IOHIDDeviceRef self = 0;
    for (CFIndex i(0); self == 0 && i < joysticksCount; ++i)
    {
        IOHIDDeviceRef d = static_cast<IOHIDDeviceRef>(const_cast<void*>(devicesArray[static_cast<size_t>(i)]));
